...
six_hr_time=0 # Enter '0' for 00Z, '6' for 06Z, '12' for 12Z or '18' for 18Z
...
ny = ovrlp_case.shape[0] 
nx = ovrlp_case.shape[1]
...
m.contour(x, y, np.squeeze(mag_wind[np.where(p==250),:,:]), contour_lvls_wind, colors=[(0,0,1)], linewidths=2)
m.contour(x, y, polj_case, colors=[(0,0.5,0)], linewidths=1)
m.contour(x, y, stj_case, colors=[(1,0,0)], linewidths=1)
m.contour(x, y, ovrlp_case, colors=[(1,1,0)], linewidths=1)
...
plt.suptitle('12Z 26 Oct. 2010 250 hPa Wind Speed (m/s) and Jet IDs (polar = green; subtropical = red; superposition = gold)') # Plot title
